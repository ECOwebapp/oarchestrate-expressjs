...
router.get("/load_all_tasks", async (req, res) => {
  ...
  if (unitRowsError) {
    console.error(
      "[AccomplishmentReport] position_of_members:",
      unitRowsError.message,
    );
    return res.status(500).json({ error: unitRowsError.message });
  }

  if (!unitRows || !unitRows.length) {
    return res.status(400).json({ error: "No unit found" });
  }
  ...
  if (!allMemberIds.length) {
    return res.status(400).json({ error: "Unit Members not found" });
  }
  ...
  if (uids.length) {
    ...
    if (profilesError) {
      console.error(
        "[AccomplishmentReport] user_profile:",
  ...
  const durDays = (s, e) => {
    const sd = new Date(s),
      ed = new Date(e);
    return isNaN(sd) || isNaN(ed)
      ? "—"
      : Math.max(1, Math.ceil((ed - sd) / 86400000));
  };
  const progressOf = (t) => {
  ...
  const mapped = approvedTasks.map((t) => {
    ...
    const resolvedUnitId =
      assigneeUnits.find((uid) => allowedUnitIds.includes(Number(uid))) ??
      assigneeUnits[0] ??
      null;
    ...
    return {
      ...
      subtaskNames: (subtaskMap[t.id] || [])
        .map((s) => s.task_profile?.title || "")
        .filter(Boolean),
      mov: t.task_output?.link
        ? { label: "View Output", url: t.task_output.link }
        : null,
      remarks: statusOf(t),
...
router.get("/load_own_tasks", async (req, res) => {
  ...
  const { data: subtasksData, error: subtasksError } = await req.supabase
    .from("subtask")
    .select(
      `id, parent_task_id, assignee, task_profile ( title, description, task_type_ref:task_type(task_type) ),
    task_approval ( unit_head, director, revised_at ),
    task_duration ( created, deadline ),
    task_output   ( link )`,
    )
  ...
  if (subtasksError) {
    console.error(
      "[IndividualAccomplishmentReport] subtasks:",
  ...
  let filteredData = [...(data || []), ...(subtasksData || [])];
  if (dateFrom && dateTo) {
    ...
    filteredData = filteredData.filter((t) => {
      ...
      return (
        checkDate(t.task_duration?.created) ||
        checkDate(t.task_duration?.deadline)
      );
    });
  }
  ...
  return res.status(200).json({
    data: filteredData.map((t) => ({
      ...
      startDate: t.task_duration?.created?.split("T")[0] || null,
      endDate: t.task_duration?.deadline?.split("T")[0] || null,
