...
import {
  resolvePosUnitIds,
  resolvePosUnitNames,
} from "../services/helperServices.js";
...
router.get("/fetch", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (isDirector) {
      ...
      if (userIds.length) {
        const { data: profRes, error: profError } = await req.supabase
          .from("user_profile")
          .select("user_id, fname, middle_initial, lname")
          .in("user_id", userIds);

        if (profError)
          console.error("[notifStore] user_profile error:", profError);
    ...
    if (isDirector) {
      ...
      if (TOOU_ID) {
        ...
        if (officeIds.length)
          officeFilter = officeIds.map((id) => `assignee.eq.${id}`).join(",");
      }

      const [{ data: d1, error: e1 }, { data: st1, error: eSt1 }] =
        await Promise.all([
          req.supabase
            .from("task")
            .select(
          ...
          req.supabase
            .from("subtask")
            .select(
              `id, parent_task_id, assignee, assigner,
            task_profile ( title, urgent, task_type_ref:task_type(task_type) ),
            task_approval ( unit_head, director ),
            task_duration ( created ),
            task_notif    ( read_by_assignee, read_by_director, read_by_unit_head ),
      ...
      const queryRes1 = [...(d1 || []), ...(st1 || [])];

      let queryRes2 = [];
      if (officeFilter) {
        const [{ data: od1 }, { data: od2 }] = await Promise.all([
          req.supabase
            .from("task")
            .select(
              `id, assignee, assigner,
        ...
        queryRes2 = [...(od1 || []), ...(od2 || [])].filter(
          (t) => t.task_output?.link && !t.task_approval?.director,
        );
      }
      const seen = new Set();
      taskRows = [...(queryRes1 || []), ...queryRes2].filter((t) => {
        if (seen.has(t.id)) return false;
        seen.add(t.id);
        return true;
      });
      taskErr = `Task: ${e1 || ""} | Subtask: ${eSt1 || ""}`;
    } else if (isUnitHead) {
      ...
      if (memberIds.length) {
        ...
        taskRows = [...(d1 || []), ...(d2 || [])].filter(
          (t) => t.task_output?.link && !t.task_approval?.unit_head,
        );
    ...
    if (uids2.length) {
      ...
      nm2 = Object.fromEntries(
        (p2 || []).map((p) => [
          p.user_id,
          resolveNames({ fname: p.fname, lname: p.lname }).trim(),
        ]),
      );
    }
    ...
    (taskRows || []).forEach((t) => {
      ...
      const taskType =
        t.task_profile?.task_type_ref?.task_type?.toLowerCase() || "regular";
      let isRead = false;
      ...
      results.push({
        id: `${t.parent_task_id ? "subtask" : "task"}-${t.id}`,
        type: t.parent_task_id ? "subtask_submitted" : "task_submitted",
    ...
    const pokerIds = [
      ...new Set((pokes || []).map((p) => p.from_user).filter(Boolean)),
    ];
    let pokerMap = {};
    if (pokerIds.length) {
    ...
    (pokes || []).forEach((p) => {
      results.push({
        ...
        body:
          p.message ||
          `Followed up on "${p.task?.task_profile?.title || "a task"}"`,
        time: p.created_at,
        read: !!p.is_read,
