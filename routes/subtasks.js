...
import { fetchSubtasks, selectAssigneeAssigner, resolveNames } from '../services/taskServices.js'
...
router.post('/upsert', async (req, res) => {
    ...
    try {
        ...
        } else if ((isSelfAssigned && subTask.type === 2) || (hasOutput && isOfficeMember)) {
            initialUnitHead = true;
        }
        ...
        if (hasOutput) {
            upserts.push(req.supabase.from('task_output').upsert({
                subtask_id: subtaskId, link: subTask.outputLink
            }, { onConflict: 'subtask_id' }));
        }

        if (subTask.oldAssignee) {
            upserts.push(req.supabase.from('subtask_assignment_log').upsert({
        ...
        const formattedSubtasks = await fetchSubtasks(req.supabase, req.user.id, null, subtaskData.parent_task_id)
        subtaskData = {}
        return res.status(200).json(formattedSubtasks);
    ...
    try {
        ...
        if (newOutputLink) {
            const { data: updated, error: updErr } = await req.supabase
                .from('task_output').update({ link: newOutputLink }).eq('subtask_id', subtaskId).select('id')
            if (updErr) throw new Error(updErr.message)
            if (!updated || updated.length === 0) {
                const { error: insErr } = await req.supabase
                    .from('task_output').insert({ subtask_id: subtaskId, link: newOutputLink })
        ...
        let dbQueries = []
        if (revisorRole === 1) {
            dbQueries.push(req.supabase.from('task_approval')
                .update({ unit_head: true, director: false, revision_comment: null, revised_at: null })
                .eq('subtask_id', subtaskId))

            if (lastRevision?.from_user) {
                dbQueries.push(req.supabase.from('task_revision').insert({
                    subtask_id: subtaskId,
                    from_user: req.user.id,
            ...
            dbQueries.push(req.supabase.from('task_notif').upsert(
            ...
            const assignerData = await selectAssigneeAssigner(req.supabase, null, subtaskId)
            ...
            dbQueries.push(_notifySubmission(
                req.supabase, null, subtaskId, assigneeId, req.user.id,
...
router.post('/delete', async (req, res) => {
    ...
    try {
        ...
        if (isMember) throw new Error('You do not have permission to delete subtasks.')
        ...
        if (isUnitHead && !isDirector) {
            ...
            if (allowedIds.length === 0) return res.status(403).json({ error: 'You can only delete subtasks that you assigned.' })

            // Optional: If you want to block the whole operation if even ONE ID is unauthorized:
            if (allowedIds.length !== subtaskIds.length) return res.status(403).json({ error: 'Unauthorized: Some subtasks were not assigned by you.' });
        }
        ...
        const del = async (table, column, ids) => {
            ...
            if (error) throw new Error(`[deleteSubtasks]: ${table} | ${error.message}`)
