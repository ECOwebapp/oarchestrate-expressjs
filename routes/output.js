...
router.post("/insert", async (req, res) => {
  ...
  try {
    ...
    const taskRow = await selectAssigneeAssigner(
      req.supabase,
      taskId,
      subtaskId,
    );
...
router.post("/update", async (req, res) => {
  ...
  try {
    const [{ data: oldOutput, error: selectErr }, { error: updErr }] =
      await Promise.all([selectQuery.maybeSingle(), updateQuery]);
