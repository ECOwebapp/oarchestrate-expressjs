...
router.post("/change_roles", async (req, res) => {
  ...
  try {
    ...
    if (Number(posId) === 0) {
      ...
      query = req.supabase.rpc("promotion", {
    ...
    const { data: updateRow, error: updateErr, status } = await query;
    if (updateErr) throw updateErr;
    if ([200, 204].some((s) => status === s)) {
      const result = await fetchMemberPos(req.supabase);
      return res.status(200).json({ data: result });
    }
  } catch (e) {
...
router.get("/fetch_unit", async (req, res) => {
  try {
    const { data, error } = await req.supabase.from("unit_name").select("*");
