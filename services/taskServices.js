...
export const fetchTasks = async (supabase, userId, taskId = null, parentId = null) => {
...
export const fetchSubtasks = async (supabase, userId, subtaskId = null, parentId = null) => {
    // 1. Get Requester Metadata
    const { isDirector, isUnitHead } = await resolvePosUnitIds(supabase, userId, null)
    const activeUnitHeadId = isUnitHead?.unit_id || null;
    ...
    if (isUnitHead) {
        ...
        const [resDirect, resParent] = await Promise.all([
            supabase.from('subtask').select(SELECT_QUERY(true)).in('assignee', allowedIds),
            supabase.from('subtask').select(SELECT_QUERY(true)).in('task.assignee', allowedIds)
        ]);
