...
export const resolvePosUnitIds = async (
  supabase,
  userId = null,
  unitId = null,
) => {
  let query = supabase.from("position").select("user_id, unit_id, pos_id");
  ...
  const directorId = userId
    ? null
    : userData.filter((p) => p.pos_id === 1)?.user_id;
